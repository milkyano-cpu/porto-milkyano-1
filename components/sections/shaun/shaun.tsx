...
// Pre-compute spoke angles for paint roller wheel — no SSR mismatch
const ROLLER_LINES = Array.from({ length: 8 }, (_, i) => {
  const angle = (i / 8) * Math.PI * 2;
  return {
    x1: +(Math.cos(angle) * 14).toFixed(3),
    y1: +(Math.sin(angle) * 14).toFixed(3),
    x2: +(Math.cos(angle) * 22).toFixed(3),
    y2: +(Math.sin(angle) * 22).toFixed(3),
  };
});
...
export default function ShaunPainting() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (cards) {
        Array.from(cards).forEach((card, i) => {
          gsap.from(card, {
            ...
            y: 50, opacity: 0, scale: 0.94, duration: 0.7,
      ...
      // Gallery
      const galItems = galleryRef.current?.querySelectorAll(".gal-item");
      if (galItems) {
        gsap.from(Array.from(galItems), {
    ...
    <div style={{ background:"#fafaf8", color:"#1a1a1a", fontFamily:"system-ui,sans-serif", overflowX:"hidden" }}>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* Paint roller rolls across hero */
        @keyframes roller-roll  {
          0%   { transform: translateX(-110%) rotate(0deg); }
          40%  { transform: translateX(0%)    rotate(-340deg); }
          100% { transform: translateX(0%)    rotate(-340deg); }
        }
        /* Paint trail reveals left to right */
        @keyframes paint-reveal {
          0%,30% { clip-path: inset(0 100% 0 0); }
          70%,100%{ clip-path: inset(0 0% 0 0); }
        }
        /* Roller idle bob */
        @keyframes roller-idle  {
        ...
        .btn-primary {
          ...
          font-size:.95rem; letter-spacing:.5px; border:none; cursor:pointer;
          border-radius:4px; transition:all .3s; text-decoration:none; white-space:nowrap;
        }
        ...
        .svc-card:hover {
          border-color:rgba(27,43,94,.35);
          transform:translateY(-4px);
          box-shadow:0 16px 48px rgba(27,43,94,.08);
        }
        .svc-card::before {
        ...
        .gal-item {
          overflow:hidden; border-radius:6px; border:1px solid var(--border);
          cursor:pointer; transition:transform .3s, box-shadow .3s; background:var(--cream3);
          position:relative;
        }
        ...
        /* roller animation classes */
        .roller-scene   { animation: roller-roll 2.4s cubic-bezier(0.25,0.46,0.45,0.94) forwards; }
        .roller-idle    { animation: roller-idle 3s ease-in-out 2.6s infinite; }
        .paint-trail    { animation: paint-reveal 2.4s ease-out forwards; }
        ...
        @media (max-width:767px) {
          ...
          .hero-section { padding:80px 20px 50px; align-items:flex-end; }
          ...
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            <div style={{ width:48,height:48,position:"relative",flexShrink:0 }}>
              <svg width="48" height="48" viewBox="0 0 48 48"
                style={{ filter:"drop-shadow(0 2px 8px rgba(27,43,94,0.2))" }}>
                {/* Hexagon badge */}
                <polygon points="24,2 43,13 43,35 24,46 5,35 5,13"
                  fill="var(--navy)" stroke="rgba(201,168,76,0.6)" strokeWidth="1.5"/>
                {/* Top highlight */}
      ...
      <section id="hero" className="hero-section" style={{ background:"linear-gradient(150deg,#fafaf8 0%,#f0eee8 50%,#fafaf8 100%)" }}>
        {/* Subtle grid */}
        <div style={{ position:"absolute",inset:0,backgroundImage:"radial-gradient(rgba(27,43,94,.06) 1px,transparent 1px)",backgroundSize:"32px 32px",pointerEvents:"none" }}/>
        {/* Navy bottom border line */}
        <div style={{ position:"absolute",bottom:0,left:0,right:0,height:3,background:"linear-gradient(90deg,transparent 0%,var(--navy) 20%,var(--navy) 80%,transparent 100%)",opacity:.15 }}/>
        ...
        <div className="hero-inner">
          ...
          <div className="hero-text">
            ...
            <div style={{ marginTop:32,display:"flex",gap:28,flexWrap:"wrap" }}>
              {[
                ...
                <a key={c.val} href={c.href} style={{ display:"flex",alignItems:"center",gap:8,textDecoration:"none",color:"var(--muted)",fontSize:".84rem",fontFamily:"'DM Sans',sans-serif",transition:"color .25s" }}
          ...
          {/* Right: Animated paint roller SVG */}
          <div ref={heroVisRef} className="hero-vis">
            <svg viewBox="0 0 480 380" fill="none" xmlns="http://www.w3.org/2000/svg" width="100%"
              style={{ overflow:"visible",filter:"drop-shadow(0 8px 40px rgba(27,43,94,0.12))" }}>

              {/* ── WALL BACKGROUND ── */}
              <rect x="20" y="30" width="440" height="300" rx="8" fill="#f0eee8" stroke="rgba(27,43,94,.1)" strokeWidth="1.5"/>

              {/* PAINT TRAIL — navy stripe that reveals left to right */}
              <g className="paint-trail">
                <rect x="20" y="120" width="440" height="90" rx="0" fill="#1B2B5E" opacity=".92"/>
                {/* Subtle sheen on painted area */}
                <rect x="20" y="120" width="440" height="18" fill="rgba(255,255,255,.07)"/>
              </g>

              {/* Wall panel lines (above and below painted strip) */}
              <line x1="20"  y1="115" x2="460" y2="115" stroke="rgba(27,43,94,.12)" strokeWidth="1"/>
              <line x1="20"  y1="215" x2="460" y2="215" stroke="rgba(27,43,94,.12)" strokeWidth="1"/>
              {/* Skirting board */}
              <rect x="20" y="305" width="440" height="25" rx="0" fill="rgba(27,43,94,.07)" stroke="rgba(27,43,94,.1)" strokeWidth="1"/>

              {/* ── PAINT ROLLER HANDLE + ROLLER ── */}
              <g className="roller-scene">
                <g className="roller-idle">
                  {/* Handle pole */}
                  <rect x="228" y="50" width="6" height="80" rx="3" fill="#8B7355" stroke="rgba(0,0,0,.15)" strokeWidth="1"/>
                  {/* Handle grip top */}
                  <rect x="220" y="44" width="22" height="12" rx="6" fill="#6B5B3E" stroke="rgba(0,0,0,.15)" strokeWidth="1"/>

                  {/* Roller frame */}
                  <rect x="194" y="124" width="68" height="14" rx="3" fill="#888" stroke="rgba(0,0,0,.2)" strokeWidth="1.5"/>
                  {/* Roller cylinder — navy paint */}
                  <rect x="192" y="118" width="72" height="46" rx="8" fill="#1B2B5E" stroke="rgba(27,43,94,.6)" strokeWidth="2"/>
                  {/* Roller sheen highlights */}
                  <rect x="194" y="120" width="68" height="8" rx="4" fill="rgba(255,255,255,.18)"/>
                  <rect x="194" y="150" width="68" height="8" rx="4" fill="rgba(0,0,0,.12)"/>
                  {/* Paint drip */}
                  <path d="M228,164 Q228,172 226,178 Q225,182 228,184 Q231,182 230,178 Q228,172 228,164Z" fill="#1B2B5E" opacity=".7"/>
                  {/* Connector arm */}
                  <rect x="226" y="128" width="6" height="8" rx="2" fill="#999"/>

                  {/* Gold accent line on roller */}
                  <rect x="192" y="138" width="72" height="3" rx="1.5" fill="rgba(201,168,76,.6)"/>

                  {/* SPS text on roller */}
                  <text x="228" y="145" textAnchor="middle" fontFamily="DM Sans,sans-serif" fontWeight="700" fontSize="11" fill="rgba(255,255,255,.85)" letterSpacing="2">SPS</text>
                </g>
              </g>

              {/* ── BEFORE/AFTER SPLIT DEMO (bottom half of wall) ── */}
              {/* Before — unpainted right side peek */}
              <rect x="320" y="230" width="140" height="70" rx="4" fill="#e8e5dc" stroke="rgba(27,43,94,.08)" strokeWidth="1"/>
              <text x="390" y="252" textAnchor="middle" fontFamily="DM Sans,sans-serif" fontSize="9" fill="rgba(26,26,26,.3)" letterSpacing="1" fontWeight="500">BEFORE</text>
              {/* Before crack lines */}
              <path d="M345,265 Q352,260 358,268 Q365,275 360,280 Q355,285 348,278Z" fill="none" stroke="rgba(26,26,26,.15)" strokeWidth="1.5" strokeLinecap="round"/>
              <path d="M410,258 Q415,263 418,270 Q421,277 416,282" fill="none" stroke="rgba(26,26,26,.12)" strokeWidth="1.2" strokeLinecap="round"/>

              {/* After — navy painted left section */}
              <rect x="20" y="230" width="140" height="70" rx="4" fill="#1B2B5E" opacity=".9"/>
              <text x="90" y="252" textAnchor="middle" fontFamily="DM Sans,sans-serif" fontSize="9" fill="rgba(255,255,255,.5)" letterSpacing="1" fontWeight="500">AFTER</text>
              {/* Gold checkmark badge */}
              <circle cx="90" cy="272" r="14" fill="rgba(201,168,76,.9)"/>
              <path d="M83,272 L88,278 L98,264" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/>

              {/* Divider line */}
              <line x1="180" y1="225" x2="180" y2="305" stroke="rgba(27,43,94,.2)" strokeWidth="2" strokeDasharray="4 3"/>
              <rect x="172" y="260" width="16" height="16" rx="8" fill="#fff" stroke="rgba(27,43,94,.2)" strokeWidth="1.5"/>
              <path d="M176,268 L179,271 L184,265" stroke="var(--navy)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/>

              {/* ── FLOATING STATS BADGES ── */}
              {/* Badge 1 */}
              <g style={{ animation:"float-badge 3s ease-in-out infinite" }}>
                <rect x="340" y="42" width="110" height="52" rx="8" fill="#fff" stroke="var(--border)" strokeWidth="1.5"
                  style={{ filter:"drop-shadow(0 4px 16px rgba(27,43,94,0.1))" }}/>
                <text x="395" y="64" textAnchor="middle" fontFamily="DM Serif Display,serif" fontSize="18" fill="var(--navy)" fontWeight="400">100%</text>
                <text x="395" y="82" textAnchor="middle" fontFamily="DM Sans,sans-serif" fontSize="9" fill="var(--muted)" letterSpacing="1" fontWeight="500">SATISFACTION</text>
              </g>
              {/* Badge 2 */}
              <g style={{ animation:"float-badge 3s ease-in-out 1.5s infinite" }}>
                <rect x="20" y="42" width="100" height="52" rx="8" fill="var(--navy)" stroke="none"
                  style={{ filter:"drop-shadow(0 4px 16px rgba(27,43,94,0.2))" }}/>
                <text x="70" y="64" textAnchor="middle" fontFamily="DM Serif Display,serif" fontSize="18" fill="#fff">Local</text>
                <text x="70" y="82" textAnchor="middle" fontFamily="DM Sans,sans-serif" fontSize="9" fill="rgba(255,255,255,.6)" letterSpacing="1" fontWeight="500">SUNSHINE COAST</text>
              </g>

              {/* ── COLOUR SWATCHES bottom right ── */}
              {[["#1B2B5E","Navy"],[  "#4a5a8a","Steel"],["#e8e5dc","Linen"],["#c9a84c","Gold"]].map(([col,name],i) => (
                <g key={name} transform={`translate(${370 + i*24},320)`}>
                  <circle cx="0" cy="0" r="10" fill={col} stroke="rgba(0,0,0,.1)" strokeWidth="1"/>
                </g>
              ))}
              <text x="395" y="350" textAnchor="middle" fontFamily="DM Sans,sans-serif" fontSize="8.5" fill="var(--muted)" letterSpacing=".5">Popular Colours</text>
      ...
      {/* ══ STATS ══ */}
      <div ref={statsRef} style={{ background:"var(--navy)",borderTop:"none",borderBottom:"none" }}>
      ...
      <section id="work" className="section-pad" style={{ background:"var(--cream2)",borderTop:"1px solid var(--border)" }}>
        <div className="inner-max">
          <div style={{ marginBottom:40 }}>
            ...
            <h2 style={{ fontFamily:"'DM Serif Display',serif",fontSize:"clamp(2rem,4.5vw,3.2rem)",color:"var(--navy)",lineHeight:.95,letterSpacing:"-1.5px" }}>
              Recent Projects<br/><em style={{ fontStyle:"italic",color:"var(--navy-l)" }}>Across the Coast</em>
            </h2>
          </div>

          <div ref={galleryRef} className="gallery-grid">

            {/* Card 1 — Feature wall (navy) */}
            <div className="gal-item">
              <svg width="100%" height="100%" viewBox="0 0 520 280" preserveAspectRatio="xMidYMid slice" style={{ display:"block" }}>
                ...
                {/* AC unit hint */}
                <rect x="30" y="40" width="80" height="30" rx="3" fill="rgba(255,255,255,.1)" stroke="rgba(255,255,255,.2)" strokeWidth="1"/>
                {/* Right wall cream */}
                <rect x="240" y="0" width="280" height="280" fill="#f0eee8"/>
                {/* Ceiling light */}
                <ellipse cx="380" cy="8" rx="30" ry="6" fill="rgba(255,240,200,.6)"/>
                <rect x="374" y="8" width="12" height="4" fill="rgba(255,240,200,.4)"/>
              </svg>
            ...
            <div className="gal-item">
              <svg width="100%" height="100%" viewBox="0 0 240 280" preserveAspectRatio="xMidYMid slice" style={{ display:"block" }}>
                <rect width="240" height="280" fill="#ede9e2"/>
                {/* Walls */}
                <rect x="0" y="0" width="240" height="260" fill="#e8e4dc"/>
            ...
            <div className="gal-item">
              <svg width="100%" height="100%" viewBox="0 0 200 210" preserveAspectRatio="xMidYMid slice" style={{ display:"block" }}>
                <rect width="200" height="210" fill="#f4f2ec"/>
                {/* Wall */}
                <rect x="0" y="0" width="200" height="210" fill="#eeece5"/>
                {/* Before: crack */}
            ...
            <div className="gal-item">
              <svg width="100%" height="100%" viewBox="0 0 200 210" preserveAspectRatio="xMidYMid slice" style={{ display:"block" }}>
                ...
                {Array.from({length:6},(_,r) => Array.from({length:4},(_,c) => (
                  <rect key={`${r}-${c}`} x={c*50+1} y={r*35+1} width="48" height="33" rx="0"
                ...
                <rect x="57" y="32" width="86" height="66" rx="2" fill="rgba(200,220,240,.12)"/>
                {/* Vanity bench */}
            ...
            {/* Card 5 — Exterior / clean lines */}
            <div className="gal-item">
              <svg width="100%" height="100%" viewBox="0 0 200 210" preserveAspectRatio="xMidYMid slice" style={{ display:"block" }}>
                <rect width="200" height="210" fill="#dde8f0"/>
                {/* Sky */}
                <rect x="0" y="0" width="200" height="120" fill="#c8dce8"/>
                {/* Ground */}
                <rect x="0" y="170" width="200" height="40" fill="#7a9c5a" opacity=".6"/>
                <rect x="0" y="180" width="200" height="30" fill="#6a8c4a" opacity=".5"/>
                {/* House */}
        ...
        <div className="why-grid">
          ...
          <div ref={whyLeftRef} style={{ background:"var(--navy)",padding:"48px 40px",position:"relative",overflow:"hidden",borderRadius:8 }}>
            ...
            <div style={{ textAlign:"center",marginBottom:32 }}>
              <svg width="120" height="120" viewBox="0 0 120 120">
                ...
                {/* S */}
                <text x="28" y="72" fontFamily="DM Serif Display,serif" fontSize="36" fill="#fff" fontWeight="400">S</text>
                {/* P with paint roller */}
                <text x="52" y="72" fontFamily="DM Serif Display,serif" fontSize="36" fill="rgba(201,168,76,.9)" fontWeight="400">P</text>
                {/* S */}
                <text x="78" y="72" fontFamily="DM Serif Display,serif" fontSize="36" fill="#fff" fontWeight="400">S</text>
                {/* paint roller replacing P crossbar */}
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
              {[
                ...
                <div key={t as string} style={{ display:"flex",gap:12 }}>
          ...
          <div ref={whyRightRef}>
            ...
            {[
              ...
              { icon:"📍", title:"Locally Based",            desc:"Northern Territory local with 15+ years in the trade. No surprises, quick response and deep knowledge of the NT climate and conditions." },
          ...
          <div className="contact-grid">
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
              {[
                ...
                { icon:"🗺️", label:"Service Area", val:"NT Wide · Darwin · Alice Springs & surrounds", href:undefined },
            ...
            <div style={{ background:"#fff",border:"1px solid var(--border)",padding:"40px 36px",borderRadius:8,boxShadow:"0 4px 32px rgba(27,43,94,.06)" }}>
              <div style={{ fontFamily:"'DM Serif Display',serif",fontSize:"1.4rem",color:"var(--navy)",marginBottom:24 }}>
              ...
              {[
                ...
                <div key={f.label} style={{ marginBottom:16 }}>
                  <label style={{ display:"block",fontSize:".65rem",letterSpacing:"1.5px",color:"rgba(26,26,26,.45)",marginBottom:6,fontFamily:"'DM Sans',sans-serif",fontWeight:600,textTransform:"uppercase" }}>
                    {f.label}
                  </label>
